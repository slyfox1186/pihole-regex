...
os.system('clear')
print('\n')
print('This script will download and add domains from the repository list to your pihole's whitelists.')
print('I am believe this is a safe list however if you run into any issues, PLEASE let me know so I can make changes!')
print('\n')
...
if os.path.exists(pihole_location):
	...
	print("[X] {} was not found. Please fix the location an re-run this script.".format(pihole_location))
...
if os.access(pihole_location, os.X_OK | os.W_OK):
	...
	print("[X] Write access is not available for {}. Please run script using root privileges." .format(
		pihole_location))
	print('\n')
	print('\n')
	exit(1)
...
if db_exists:
	...
	try: # Try to create a DB connection
		...
		gravityScript_before = cursor.execute(" SELECT * FROM domainlist WHERE type = 0 AND comment LIKE '%sly9zk%' ")
		...
		user_add = cursor.execute(" SELECT * FROM domainlist WHERE type = 0 AND comment NOT LIKE '%sly9zk%' ")
		...
		if uagl == True: # if we found user added domains from our list in gravity do:
			print ('[i] {} Domain(s) that already exist and would be duplicates if they were still added by this script.\n' .format(INgravityUSERaddListCount+1)) # we have to add 1 for humans cause count starts @ 0
			a = 0
			...
			print ('[i] This script found {} domains that will be added since they don't already exist.' .format(INgravityUSERaddListCount)) # notify of negative result
		#
		#
		...
		print('[i] Checking Gravity for domains that have been added since this script was last run.')
		ignl = False
		a = 0
		...
		if ignl == True:
			print ('[i] Since this scripts last run, {} domain(s) no are longer included in the list and will be removed.\n' .format(INgravityNOTnewListCount+1))
			a = 0
			while z >= 0:
				a += 1
		...
		# If not keep going
			...
			print ('[i] {} domain(s) have been added by OTHER methods and are also NOT included in this script.' .format(INgravityNOTnewListCount))
		#
		#
		...
		if ignl == True:
			print('\n')
		#
		print('[i] Checking Gravity's database for missing domains.')
		ilng = False
		...
		if ilng == True: # Add domains that are missing from new script and not user additions
			print ('[i] {} domain(s) are missing from Gravity's database and will be added by this script.\n' .format(INnewNOTgravityListCount+1))
			a = 0
			while w >= 0:
				a += 1
			...
			gravityScript_after = cursor.execute(" SELECT * FROM domainlist WHERE type = 0 AND comment LIKE '%sly9zk%' ")
			# fetch all matching entries which will create a tuple for us
			...
			if gsa == True:
				...
				print("\n[i] All {} missing domain(s) were added to Gravity's database." .format(newWhiteListlen))

			else:
				print ("\n[i] All {} newly found domain(s) have not been added to Gravity." .format(INnewNOTgravityListCount+1))

		else: # We should be done now
			# Do Nothing and exit. All domains are accounted for.
			print("[i] All {} domains that should have been added were successfully found in Gravity." .format(newWhiteListlen)) 
		...
		if ilng == True:
			print('[i] Now restarting Pi-hole to update the Gravity database. Please be patient because this could take a moment.')
		...
		if os.path.isfile(slyfox1186_whitelist_location) and os.path.getsize(slyfox1186_whitelist_location) > 0:
			...
			with open(slyfox1186_whitelist_location, 'r') as fOpen:
				...
				if whitelist_old_slyfox1186:
					print('[i] Located a whitelist from a previous run and will remove it before continuing.')
	...
	print('[i] : Important!: Please alert me of any issues you discover so I can fix them!')
	print('\n')
