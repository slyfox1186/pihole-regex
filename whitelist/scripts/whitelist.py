#!/usr/bin/env python3
# Project homepage: https://github.com/slyfox1186/pihole.youtube.blocklist/
# ================================================================================
import os
import argparse
import sqlite3
...
parser.add_argument(
    "-d", "--dir", type=dir_path, help="optional: Pi-hole etc directory"
)
parser.add_argument(
    "-D",
    "--docker",
    action="store_true",
    help="optional: set if you're using Pi-hole in docker environment",
)
...
whitelist_remote_url = "https://raw.githubusercontent.com/slyfox1186/pihole.youtube.blocklist/main/whitelist/scripts/lists/whitelist.txt"
remote_sql_url = "https://raw.githubusercontent.com/slyfox1186/pihole.youtube.blocklist/main/whitelist/scripts/lists/whitelist.sql"
...
slyfox1186_whitelist_location = os.path.join(pihole_location, "slyfox-whitelist.txt")
...
print("\n")
print(
    "This script will download and add domains from the repository list to your piholes whitelists."
)
print(
    "I am believe this is a safe list however if you run into any issues, PLEASE let me know so I can make changes!"
)
print("\n")

# Check for pihole path exsists
if os.path.exists(pihole_location):
    print("[i] Your Pi-hole path was found.")
else:
    # print(f'[X] {pihole_location} was not found. Please fix the location an re-run this script.')

    print(
        "[X] {} was not found. Please fix the location an re-run this script.".format(
            pihole_location
        )
    )
...
if os.access(pihole_location, os.X_OK | os.W_OK):
    ...
    print(
        "[X] Write access is not available for {}. Please run script using root privileges.".format(
            pihole_location
        )
    )
    print("\n")
...
if os.path.isfile(gravity_db_location) and os.path.getsize(gravity_db_location) > 0:
    db_exists = True
    print("[i] The Gravity database file was found")
    ...
    if len(remote_sql_str) > 0:
        print(
            "[i] {} domains and {} SQL queries discovered".format(
...
if whitelist_str:
    whitelist_remote.update(
        x for x in map(str.strip, whitelist_str.splitlines()) if x and x[:1] != "#"
    )
else:
...
if db_exists:
    ...
    try:  # Try to create a DB connection
        ...
        print(
            "[i] {} Domains from script in whitelist already.".format(
                gravScriptBeforeTUPlen
            )
        )
        #
        ...
        for newWhiteDomain in newWhiteTUP:  # for each line found domains.sql
            ...
            newWhiteList[nwl] = newWL[1].replace(
                "'", ""
            )  # remove ' from domain and add to list
        ...
        user_add = cursor.execute(
            " SELECT * FROM domainlist WHERE type = 0 AND comment NOT LIKE '%sly9zk%' "
        )
        ...
        userAddList = [
            None
        ] * userAddTUPlen  # create a list that has the same size as the tuple is it compared to
        uA = 0  # used to count User Added domains in our script
        uagl = False
        for (
            userAddINgravity
        ) in userAddTUP:  # for every whitelisted domain we found in the database do:
            if (
                userAddINgravity[2] in newWhiteList
            ):  # if the domain we found added by user IS IN our new list count it
                userAddList[uA] = userAddINgravity[
                    2
                ]  # add the domain we found to the list we created
                uagl = True
                uA += 1  # bump count if domain added (starts @ 0)
        #
        uA -= 1  # subtract 1 so the count doesn't put us out of range
        INgravityUSERaddListCount = (
            uA  # save our count here so we know how many we have later
        )
        ...
        if uagl == True:  # if we found user added domains from our list in gravity do:
            print(
                "[i] {} Domain(s) that already exist and would be duplicates if they were still added by this script.\n".format(
                    INgravityUSERaddListCount + 1
                )
            )  # we have to add 1 for humans cause count starts @ 0
            a = 0
            ...
            print(
                "[i] This script found {} domains that will be added since they do not already exist.".format(
                    INgravityUSERaddListCount
                )
            )  # notify of negative result
        #
        #
        # Check Gravity database for domains added by script that are not in new script
        INgravityNOTnewList = [
            None
        ] * gravScriptBeforeTUPlen  # create a list that has the same size as the tuple is it compared to
        ...
        print(
            "[i] Checking Gravity for domains that have been added since this script was last run."
        )
        ignl = False
        a = 0
        for (
            ...
            gravScriptBeforeList[a] = INgravityNOTnew[
                2
            ]  # take domains from gravity and put them in a list for later
            a += 1
            if (
                not INgravityNOTnew[2] in newWhiteList
            ):  # make sure it is not in new script
                INgravityNOTnewList[
                    z
                ] = INgravityNOTnew  # add not found to list for later
                ignl = True
                z += 1
        #
        z -= 1
        ...
        if ignl == True:
            print(
                "[i] Since this scripts last run, {} domain(s) no are longer included in the list and will be removed.\n".format(
                    INgravityNOTnewListCount + 1
                )
            )
            a = 0
            while z >= 0:
                ...
                sql_delete = (
                    " DELETE FROM domainlist WHERE type = 0 AND id = '{}' ".format(
                        INgravityNOTnewList[z][0]
                    )
                )
        ...
        # If not keep going
            ...
            print(
                "[i] {} domain(s) have been added by OTHER methods and are also NOT included in this script.".format(
                    INgravityNOTnewListCount
                )
            )
        #
        #
        ...
        if ignl == True:
            print("\n")
        #
        print("[i] Checking the Gravity database for missing domains.")
        ilng = False
        for INnewNOTgravity in newWhiteList:  # for every domain in the new script
            if (
                not INnewNOTgravity in gravScriptBeforeList
                and not INnewNOTgravity in userAddList
            ):  # make sure it is not in gravity or added by user
                INnewNOTgravityList[
                    w
                ] = INnewNOTgravity  # add domain to list we created
                ilng = True
                w += 1
        #
        w -= 1
        ...
        if (
            ilng == True
        ):  # Add domains that are missing from new script and not user additions
            print(
                "[i] {} domain(s) are missing from the Gravitys database and will be added by this script.\n".format(
                    INnewNOTgravityListCount + 1
                )
            )
            a = 0
            while w >= 0:
                ...
                for addNewWhiteDomain in newWhiteList:
                    if addNewWhiteDomain in INnewNOTgravityList:
                        ...
                        # print (nW[sql_index])
                        # ability to add new
                        sql_add = " INSERT OR IGNORE INTO domainlist (type, domain, enabled, comment) VALUES {} ".format(
                            nW[sql_index]
                        )
                        cursor.executescript(sql_add)
                        w -= 1
            # Re-Check Gravity database for domains added by script after we update it
            gravityScript_after = cursor.execute(
                " SELECT * FROM domainlist WHERE type = 0 AND comment LIKE '%sly9zk%' "
            )
            # fetch all matching entries which will create a tuple for us
            ...
            if gsa == True:
                ...
                print(
                    "\n[i] All {} missing domain(s) were added to Gravity's database.".format(
                        newWhiteListlen
                    )
                )
                ...
                print(
                    "\n[i] All {} newly found domain(s) have not been added to Gravity.".format(
                        INnewNOTgravityListCount + 1
                    )
                )
            ...
            print(
                "[i] All {} domains that should have been added were successfully found in Gravity.".format(
                    newWhiteListlen
                )
            )
        ...
        print(
            "[i] There are a total of {} domains in your whitelist (regex({}) & exact({}))".format(
                total_domains, tdr, tde
            )
        )
        ...
        if ilng == True:
            print(
                "[i] Now restarting Pi-hole to update the Gravity database. Please be patient because this could take a moment."
            )
            restart_pihole(args.docker)

    except sqlite3.Error as error:
        print("[X] This script FAILED to make changes to the Gravity database.")
        print("\n")
        print("\n")
        exit(1)
    ...
    if (
        os.path.isfile(gravity_whitelist_location)
        and os.path.getsize(gravity_whitelist_location) > 0
    ):
        print("[i] Gathering the domains listed in the whitelist file.")
        with open(gravity_whitelist_location, "r") as fRead:
            whitelist_local.update(
                x for x in map(str.strip, fRead) if x and x[:1] != "#"
            )

    if whitelist_local:
        print("[i] {} existing whitelists identified".format(len(whitelist_local)))
        if (
            os.path.isfile(slyfox1186_whitelist_location)
            and os.path.getsize(slyfox1186_whitelist_location) > 0
        ):
            print("[i] Existing slyfox1186-whitelist install identified")
            with open(slyfox1186_whitelist_location, "r") as fOpen:
                whitelist_old_slyfox1186.update(
                    x for x in map(str.strip, fOpen) if x and x[:1] != "#"
                )

                if whitelist_old_slyfox1186:
                    print(
                        "[i] Located a whitelist from a previous run and will remove it before continuing."
                    )
    ...
    print(
        "[i] Outputting {} domains to {}".format(
    ...
    print("[i] The domain changes are complete.\n")
    print("[i] Now restarting Pi-hole. Please be patient while this completes.")
    restart_pihole(args.docker)
    print("[i] Restart complete!")
    print(
        "[i] : Important!: Please alert me of any issues you discover so I can fix them!"
    )
    print("\n")
