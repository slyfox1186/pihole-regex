...
def update_database(db_path, domains):
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            for domain in domains:
                ...
                cursor.execute("INSERT OR IGNORE INTO domainlist (type, domain, enabled, date_added, comment) VALUES (?, ?, ?, ?, ?)", 
                               (0, domain, 1, int(time.time()), "SlyEWL - github.com/slyfox1186/pihole-exact"))
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            cursor = conn.cursor()
            for domain in domains:
                cursor.execute("INSERT OR IGNORE INTO domainlist (type, domain, enabled, date_added, comment) VALUES (?, ?, ?, ?, ?)", (0, domain, 1, int(time.time()), "Added exact: "))
                print(f"Added exact: {domain}")
    except sqlite3.Error as e:
        print(f"[X] Database error: {e}")
        return
...
def main():
    ...
    whitelist_url = "https://raw.githubusercontent.com/slyfox1186/pihole-exact/main/domains/whitelist/exact-whitelist.txt"  # Replace with actual URL
