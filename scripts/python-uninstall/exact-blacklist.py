...
if db_exists:
    ...
    try: # Try to create a database connection
        ...
        gravity_script_before = cursor.execute(" SELECT * FROM domainlist WHERE type = 1 AND comment LIKE '%SlyEBL%' ")
        # Fetch all matching entries which will create a tuple for us
        GravScript_BeforeTUP = gravity_script_before.fetchall()
        ...
        if ignl == True:
            print("[i] {} domain(s) added previously by script that are not in new script." .format(InGravityNotNewListCount+1))
            a = 0
            while z >= 0:
                a += 1
        ...
        if ilng == True: # add domains that are missing from new script and not user additions
            print("[i] {} domain(s) NOT in Gravity that are in new script." .format(InNew_NotGravityListCount+1))
            a = 0
            while w >= 0:
                a += 1
            ...
            gravity_script_After = cursor.execute(" SELECT * FROM domainlist WHERE type = 1 AND comment LIKE '%SlyEBL%' ")
            # Fetch all matching entries which will create a tuple for us
            GravScript_AfterTUP = gravity_script_After.fetchall()
            # Number of domains in database from script
        ...
        print('[i] The Exact Blacklist filters were removed from Pi-Hole.\n')
