...
SQL_FILE_URL = 'https://raw.githubusercontent.com/slyfox1186/pihole-regex/main/domains/exact-blacklist.sql'
...
def update_pihole_db(domains_to_update):
    ...
    for domain, comment in domains_to_update.items():
        if domain in existing_domains:
            ...
            cursor.execute("INSERT INTO domainlist (type, domain, comment) VALUES (?, ?, ?)", (1, domain, comment))
...
def main():
    try:
        ...
        if removed:
            ...
            print("\nNo domains were removed from the exact blacklist.\n")

        if not added and not removed:
            print("\nNo changes were required for the exact blacklist.\n")
